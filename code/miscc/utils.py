...
#     # Ensure you have the correct path to your font file, e.g., '/path/to/arial.ttf'
...
#             if cap[j] == 0:
#                 break
#             # Directly use the word without unnecessary encoding/decoding
#             word = ixtoword[cap[j]]
#             d.text(((j + off1) * (vis_size + off2), i * FONT_MAX), f'{j}:{word[:6]}',
#                    font=fnt, fill=(255, 255, 255, 255))  # Ensure FONT_MAX is defined or replace with actual max font height
#             sentence.append(word)
#         sentence_list.append(sentence)
        ...
        for j in range(num_attn):
            ...
            if (vis_size // att_sze) > 1:
               # one_map = \
                ...
                # expanded_image = pyramid_expand(one_map, sigma=20, upscale=vis_size // att_sze)
            minV = one_map.min()
            maxV = one_map.max()
